...
.then(() => {
  return Templates.create(components)
})
.then(templatesCreated => {
  console.log(`${templatesCreated.length} templates created with the following id:`);
  console.log(templatesCreated.map(c => c._id));
})
.then(() => {
